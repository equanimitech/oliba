...
- The sitting is written up as one self-contained HTML lesson: think-first prompts, contrast notes for the misconceptions you corrected, instant-feedback quizzes. It is a record you can reopen, practise in and share as a single file. Each quiz item is rewritten as a production card, so the page's recognition practice continues as recall in `/study`.
