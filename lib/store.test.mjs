...
beforeEach(() => {
  tempRoot = mkdtempSync(join(tmpdir(), 'oliba-store-'));
  process.env.OLIBA_DIR = join(tempRoot, 'data');
});

afterEach(() => {
  delete process.env.OLIBA_DIR;
  rmSync(tempRoot, { recursive: true, force: true });
});

test('dataDir honors the OLIBA_DIR override', () => {
