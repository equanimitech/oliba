...
const runScript = (script, args = [], input) =>
  execFileSync(process.execPath, [script, ...args], {
    env: { ...process.env, OLIBA_DIR: dir, CLAUDE_CONFIG_DIR: join(dir, 'claude') },
    encoding: 'utf8',
    input,
  });
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'oliba-hud-'));
...
test('statusline stashes the cue card id for /study', () => {
...
test('hook prints the Node message instead of crashing when node is missing', () => {
  ...
  const run = (/** @type {string} */ path) => execFileSync('/bin/sh', ['-c', command], {
    env: { PATH: path, CLAUDE_PLUGIN_ROOT: join(LIB, '..'), OLIBA_DIR: dir, CLAUDE_CONFIG_DIR: join(dir, 'claude') },
    encoding: 'utf8',
  });
  assert.match(run('/usr/bin:/bin'), /needs Node\.js/);
  assert.match(run(`${dirname(process.execPath)}:/usr/bin:/bin`), /oliba active/);
});

test('session-start names exactly the four skills', () => {
  const out = runScript(SESSION_START);
  for (const skill of ['/oliba:syllabus', '/oliba:teach', '/oliba:study', '/oliba:config']) assert.ok(out.includes(skill), skill);
  for (const gone of ['/deep-lesson', '/answer', '/sift', '/card', '/harvest', '/read', '/lesson']) assert.ok(!out.includes(gone), gone);
});
