...
/**
 * Rate a lesson's quiz cards from the page's first-attempt results:
 * wrong → again, right → good. Either `--file <oliba-results-*.json>` (the
 * page's download, deleted once imported) or inline `<projectId> <lesson> q1:✗ q2:✓`.
 * A card reviewed after the answers (or, inline, after the lesson was written)
 * is skipped, so importing twice never rates twice.
 * @param {string[]} argv
 */
function cmdResultsImport(argv) {
  ...
  /** @type {{ project: string, lesson: string, answeredAt?: string, items: { q: string, correct: boolean }[] }} */
  let results;
  if (values.file) {
    try {
      results = JSON.parse(readFileSync(values.file, 'utf8'));
    } catch {
      fail(`cannot read results from ${values.file}`);
    }
    if (!results?.project || !results.lesson || !Array.isArray(results.items)) fail(`${values.file} is not an oliba results file`);
  } else {
    ...
    results = { project, lesson, items };
  }

  const projects = readProjects();
  const project = projects[results.project];
  if (!project) fail(`no project with id ${results.project}`);
  /** @param {string} path */
  ...
  /** @type {{ q: string, cardId: string, rating: string }[]} */
  const rated = [];
  ...
  for (const { q, correct } of results.items) {
    ...
    if (!card) {
      ...
      cards[card.id] = { ...card, fsrs: review(card.fsrs, RATING_NAMES[rating]) };
      rated.push({ q, cardId: card.id, rating });
    }
  }
  writeCards(cards);
  if (values.file && /^oliba-results-.*\.json$/.test(base(values.file) ?? '')) {
    try { rmSync(values.file); } catch { /* already gone */ }
  }
  out({ rated, skipped });
}
